...
export function TradeBasket({
  ...
  async function completeTrade() {
    ...
    setBusy(false);
    if (!result) {
      setError("The trade could not be completed. Review the trade pile and try again.");
      return;
    }
    setCompleted(result);
    setStep("complete");
  ...
  return (
    <div className="basket-backdrop" role="presentation">
      <section className="basket-mode" role="dialog" aria-modal="true" aria-labelledby="basket-title">
        <header className="basket-header"><button onClick={step === "confirm" ? () => setStep("basket") : onClose} aria-label="Close Trade Pile">{step === "confirm" ? "←" : "×"}</button><div><p>{step === "confirm" ? "FINAL CHECK" : "STICKERS YOU'RE GIVING"}</p><h2 id="basket-title">{step === "confirm" ? "Review Trade" : "Trade Pile"}</h2></div><span>{total}<small>stickers</small></span></header>

        {step === "complete" && completed ? (
          <div className="trade-complete-state"><span>✓</span><h2>Trade complete</h2><p>{completed.total} sticker{completed.total === 1 ? " was" : "s were"} removed from Extras. Your trade pile is now empty.</p>{error && <small>{error}</small>}<button onClick={() => void undoTrade()} disabled={busy}>↶ Undo Trade</button><button onClick={onClose}>Done</button><em>Undo is available for 5 minutes.</em></div>
        ) : (
          <div className="basket-body">
            ...
            {groups.length ? <div className={`basket-groups ${step === "confirm" ? "confirming" : ""}`}>{groups.map(({ section, items: groupItems }) => (
              <section className="basket-group" key={section.code}><header><span>{section.code}</span><strong>{section.name}</strong><b><FlagIcon code={section.code} fallback={section.flag} /></b></header><div>{groupItems.map((item) => {
                ...
                return <article key={item.code}><span className="basket-sticker-code"><strong>{item.displayCode}</strong><small>{step === "confirm" ? `Giving ${item.reserved}` : `${item.reserved} in this trade`}</small></span>{step === "confirm" ? <b className="confirm-quantity">×{item.reserved}</b> : <><div className="basket-quantity"><button onClick={() => void onAdjust(item.code, -1)} aria-label={`Decrease ${item.displayCode} in trade pile`}>−</button><strong>{item.reserved}</strong><button disabled={available < 1} onClick={() => void onAdjust(item.code, 1)} aria-label={`Increase ${item.displayCode} in trade pile`}>＋</button></div><button className="remove-basket-item" onClick={() => void removeAll(item.code, item.reserved)}>Remove</button></>}</article>;
              })}</div></section>
            ))}</div> : <div className="empty-basket"><span>▱</span><h2>Your trade pile is empty</h2><p>Add extras to start building a trade.</p></div>}
            {error && <p className="basket-error">{error}</p>}
            <footer className="basket-footer">{step === "confirm" ? <><button onClick={() => setStep("basket")}>Cancel</button><button disabled={!total || busy} onClick={() => void completeTrade()}>{busy ? "Completing…" : `Confirm ${total} Sticker${total === 1 ? "" : "s"}`}</button></> : <><button disabled={!total || busy} onClick={() => void onClear()}>Clear Trade Pile</button><button disabled={!total || busy} onClick={() => setStep("confirm")}>Complete Trade</button></>}</footer>
          </div>
        )}
      </section>
    </div>
